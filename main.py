...
NUM_WORKERS = 3
conn = psycopg2.connect(host='localhost',database="postgres",port="5432",user="postgres",password="postgres")
cursor = conn.cursor()
...
class ProcessVideo:
    ...
    def drawDeleaunay(self, image, subdiv):
        ...
        #updating the db to indicate the frame processing has started
        size = image.shape
        r = (0, 0, size[1], size[0])
    ...
    def generateFramesInRange(self, imageList, startIdx, endIdx):
        logger.info(f"Worker generating delaunay triangles for frames ranging {startIdx} - {endIdx}")
        for i in range(startIdx, endIdx):
            ...
            # genrate vertices
            logger.debug(f"Worker generating vertices for frame {i}")
            ...
            for k in range(0, len(allFaces)):
                ...
                #query to add vertices to the master table

                processingQuery = "update master_table set datapoints='"+str(verticesfilePath)+"' where taskid = '"+str(self.videoIDStr)+"_"+i+"';"
                cursor.execute(processingQuery)
                conn.commit()
            ...
            for i in range(startIdx,endIdx):
                processingQuery="update master_table set jobstatus = 'Processing' where taskid ='"+str(self.videoIDStr)+"_"+i+"';"
                cursor.execute(processingQuery)
                conn.commit() 
        ...
        for i in range(NUM_WORKERS - 1):
            ...
            if end_idx != 0:
                ...
                for x in range(start_idx,end_idx):
                    processingQuery ="insert into master_table values('"+str(self.videoIDStr)+"_"+str(x)+"','"+str(self.videoIDStr)+"','"+str(x)+"','"+str(i)+"',null,'Assigned',null);"
                    print(processingQuery)
                    cursor.execute(processingQuery)
                    conn.commit()
                    
                    print("update master_table set assignednode = 'worker_"+str(i)+"' where master_table.taskid = '"+str(self.videoIDStr)+"_"+str(x)+"';")
                    processingQuery2= "update master_table set assignednode= 'worker_"+str(i)+"' where master_table.taskid = '"+str(self.videoIDStr)+"_"+str(x)+"';"
                    print(processingQuery2) 

                    cursor.execute(processingQuery2)
                    conn.commit() 
    ...
    def generateOutputVideo(self):
        videofilename = "output_" + self.videoIDStr + ".mkv"
        VIDEO_DIR.mkdir(parents=True, exist_ok=True)
        videoPath = VIDEO_DIR/videofilename
        meshframesPath = MESH_DIR/self.videoIDStr
        imageList = sorted(meshframesPath.glob('*.jpg'))

        #query to add output file path
        processingQuery = "update master_table set outputfielname = '"+str(videofilename)+"' where taskid like '"+self.videoIDStr+"%';" 
        cursor.execute(processingQuery)
        conn.commit()
...
def main():    
    ...
    # This extracts frames from video
    videoProcessor.extractFramesFromVideo()
    #Database notify next step
    logger.info("Completed frames extraction")
    

    logger.info("Start generating delaunay triangles")
    # databse listen for previous step
    #This spaws worker threads to generate vertices, draw triangles and finally output mesh frames
    ...
    # databse listen for previous step
    # This step combines the generated mesh frames from previous step into a video file
